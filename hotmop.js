...
//
// i modified it to use syntax-error on the file before reload
//
//   https://www.npmjs.com/package/syntax-error
//
// if there's a syntax error, the module emits an 'error' event
//
// also, the module now returns a function that takes an app path
//   
//
//   var hotswap = require('./hotswap')(appPath)
//
// it returns hotswap, an event emitter.
//
var async = require('async');
var fs = require('fs');
var path = require('path');
var Events = require('events');
// check for errors mod
...
function setup () {
  ...
  // we get the filenames for hot-swapping as args
  // here's an inelegant spread operator to deal with that
  var filenames =  []
  ...
  function extension_js(type, module, filename)
  {
    ...
    is_hotswap_file = filenames.indexOf(filename) > -1 // we hotswap it if it got passed in to the module
    ...
    if (typeof(newmodule.exports) != 'object' && typeof(newmodule.exports) != 'function') {
      ...
      if (oldmodule === undefined) {
        ...
        emitter.emit(filename) // on swap, will emit the filename being swapped
      }
    }
